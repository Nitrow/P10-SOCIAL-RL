...
class P10_DRL_Lvl3_Grasping_Primitives_Conveyor(gym.Env):
    ...
    def step(self, action):
        ...
        # Set actions
        for i in range(action): self.supervisor.step(self.TIME_STEP)
    ...
    def getState(self, mode="default"):
        ...
        state["Timing"] += [float(round(x)) for x in R.from_matrix(np.array(self.goal_node.getOrientation()).reshape(3,3)).as_euler('ZYX', degrees=True)]
