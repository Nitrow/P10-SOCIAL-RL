...
from P10_DRL_Lvl3_Grasping_Combined.envs import P10_DRL_Lvl3_Grasping_Combined
...
if __name__ == '__main__':
    ...
    itername="stage2_testRotations_FixConvSpeed_20-120cmspawn_" + str(n_games) + "episodes_"+ str(batchsize)+ "batchsize_lr001" + str(neurons) + "neurons" # alwaysreset
    
    env = P10_DRL_Lvl3_Grasping_Combined(itername, vdist, plt_avg)
    shutil.copy(env.own_path, env.path)
    agent = Agent(gamma=0, epsilon=1.0, batch_size=batchsize, n_actions=env.action_shape, eps_end=0.01, input_dims=[env.state_shape], lr=lr, chkpt_dir=env.path, fc1_dims=neurons, fc2_dims=neurons)
    scores, eps_history = [], []
