...
class P10_DRL_Lvl3_Grasping_Primitives(gym.Env):
    ...
    def getState(self):
        ...
        state += [round(x) for x in R.from_matrix(np.array(self.tcp.getOrientation()).reshape(3,3)).as_euler('ZYX', degrees=True)]  # Get joint angle
        print([round(x) for x in R.from_matrix(np.array(self.goal_node.getOrientation()).reshape(3,3)).as_euler('ZYX', degrees=True)])
        state += [round(x) for x in R.from_matrix(np.array(self.goal_node.getOrientation()).reshape(3,3)).as_euler('ZYX', degrees=True)]
        #state.append(x_dist)
        #state.append(y_dist)
