...
class P10_DRL_Lvl3_Grasping_Primitives(gym.Env):
    ...
    def getState(self):
        ...
        state = []
        #state.append(self.sensors[-1].getValue())  # Get joint angle  # Get joint angle
        
        state += np.matmul(np.transpose(np.array(self.tcp.getOrientation())), np.array(self.goal_node.getOrientation()))
        print(np.matmul(np.transpose(np.array(self.tcp.getOrientation())), np.array(self.goal_node.getOrientation())))
        state += [x_dist]
        state += [y_dist]
