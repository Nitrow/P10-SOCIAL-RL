...
class P10_DRL_Lvl3_Grasping_Primitives(gym.Env):
    ...
    def __init__(self, itername, vdist, plt_avg):
        ...
        self.actions = [m.radians((180/self.action_shape)*i) for i in range(self.action_shape)]
        self.state_shape = 13
    ...
    def step(self, action):
        ...
        self.angleDiff = -abs(m.radians(can_angle - tcp_angle))
        self._setTarget()
        self._action_moveTCP(1)
        for i in range(50): self.supervisor.step(self.TIME_STEP)
        state = self.getState()
        ...
        return [state, float(self.angleDiff), self.done, {}]
    ...
    def _setTarget(self):
        ...
        #z = random.uniform(0.4 - self.vdist, 0.4 + self.vdist/2)
        z_pos = random.choice(["far", "close"])
        x_pos = random.choice(["left", "mid", "right"])
        #z_pos = random.choice(["close"])#, "close"])
        #x_pos = random.choice(["right"])#, "mid", "right"])
        
        self.chosenPos = z_pos + "-" + x_pos
        self.up_pose = self.poses["up-" + self.chosenPos]
        self.down_pose = self.poses["down-" + self.chosenPos]
        x = self.pose_code[x_pos]
        z = self.pose_code[z_pos]
    ...
    def getState(self):
        ...
        state += self.tcp.getField("rotation").getSFRotation()
        state += self.goal_node.getField("rotation").getSFRotation()
        state += self.up_pose
        #state.append(self.sensors[-1].getValue())  # Get joint angle
        # print("TCP orientation: {}".format([round(x) for x in R.from_matrix(np.array(self.tcp.getOrientation()).reshape(3,3)).as_euler('ZYX', degrees=True)]))
        # print("GOAL orientation: {}".format([round(x) for x in R.from_matrix(np.array(self.goal_node.getOrientation()).reshape(3,3)).as_euler('ZYX', degrees=True)]))
        # print("******************")
        #state += [[round(x) for x in R.from_matrix(np.array(self.tcp.getOrientation()).reshape(3,3)).as_euler('ZYX', degrees=True)][1]]  # Get joint angle
        #state += [[round(x) for x in R.from_matrix(np.array(self.goal_node.getOrientation()).reshape(3,3)).as_euler('ZYX', degrees=True)][2]]
        #state.append(x_dist)
        #state.append(y_dist)
